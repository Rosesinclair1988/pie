...
class Quadra_Atos_EuroController extends Mage_Core_Controller_Front_Action
{
    ...
    protected $_atosEuroResponse = null;
    ...
    public function getQuote()
    {
        if (!$this->_quote) {
            ...
            $this->_quote = Mage::getModel('sales/quote')->load($session->getAtosEuroQuoteId());
    ...
    public function getRealOrderIds()
    {
        if (!$this->_realOrderIds) {
            if ($this->_atosEuroResponse) {
                $this->_realOrderIds = explode(',', $this->_atosEuroResponse['order_id']);
            } else {
                return array();
            }
        }
    ...
    {
        if (count($response)) {
            $this->_atosEuroResponse = $response;
        }
        return $this;
    }
